...
def one(input1, input2):
        return max(input1, input2, key=lambda x:len(x)) if len(input1)!=len(input2) else ' '.join([input1, input2])
...
def three(arg1):
        result = ''
        if not arg1%3: result+='fizz'
        if not arg1%5: result+='buzz'
...
def four(arg1):
        return max([sum(map(lambda x:int(x), num)) for num in arg1.split() if num.isdigit()] or [0])
...
def five(input):
        return list(dict(map(lambda x:(x,None), [input.split(',')[i:i+4][0] for i in range(0, len(input.split(',')), 4) if input.split(',')[i:i+4][2]=='False'])))
...
def seven(input):
        return sum(input.lower().count(x) for x in ['a','e','i','o','u'])
...
def eight(input):
        product = 1
        for x in range(1, input+1): product *= x
        return product
...
def nine(inputString, char):
        return inputString.replace(' ','').index(char)+1 if char in inputString else -1
...
def ten(string, int, char):
        return string[int-1].lower()==char if int<len(string) else False
